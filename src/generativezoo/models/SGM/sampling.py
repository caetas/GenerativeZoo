...
def get_predictor(name):
  if name.lower() == 'none':
    ...
    raise ValueError(f"Predictor name {name} unknown.")
...
def get_corrector(name):
  if name.lower() == 'none':
    ...
    raise ValueError(f"Corrector name {name} unknown.")


def get_sampling_fn(args, sde, shape, eps):
  ...
  if sampler_name.lower() == 'ode':
    sampling_fn = get_ode_sampler(sde=sde,
                                  shape=shape,
                                  denoise=args.noise_removal,
                                  eps=eps,
                                  device=device)
  # Predictor-Corrector sampling. Predictor-only and Corrector-only samplers are special cases.
  elif sampler_name.lower() == 'pc':
    predictor = get_predictor(args.predictor.lower())
    corrector = get_corrector(args.corrector.lower())
