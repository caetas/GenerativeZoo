from models.GANs.AdversarialVAE import *
...
if args.train:
    ...
    train_loader, input_size, channels = pick_dataset(dataset_name=args.dataset, batch_size=args.batch_size, normalize=True)
    val_loader, _, _ = pick_dataset(dataset_name=args.dataset, batch_size=args.batch_size, normalize=True, mode='train')
    ...
    test_loader, input_size, channels = pick_dataset(dataset_name=args.dataset, batch_size=args.batch_size, normalize=True, mode='val')
    model = AdversarialVAE(input_shape = input_size, device = device, input_channels = channels, latent_dim = args.latent_dim, n_epochs = args.n_epochs, hidden_dims = args.hidden_dims, lr = args.lr, batch_size = args.batch_size, gen_weight = args.gen_weight, recon_weight=args.recon_weight, sample_and_save_frequency = args.sample_and_save_frequency)
    model.load_state_dict(torch.load(args.checkpoint))
    model.create_validation_grid(test_loader)
elif args.sample:
    _, input_size, channels = pick_dataset(dataset_name=args.dataset, batch_size=args.batch_size, normalize=True, mode='val')
    model = AdversarialVAE(input_shape = input_size, device = device, input_channels = channels, latent_dim = args.latent_dim, n_epochs = args.n_epochs, hidden_dims = args.hidden_dims, lr = args.lr, batch_size = args.batch_size, gen_weight = args.gen_weight, recon_weight=args.recon_weight, sample_and_save_frequency = args.sample_and_save_frequency)
    model.load_state_dict(torch.load(args.checkpoint))
    model.create_grid()
else:
